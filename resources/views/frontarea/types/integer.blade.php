            ...
            @foreach($values as $value)
                ...
                <div class="entry form-group{{ $errors->has($attribute->slug.'.'.$loop->index) ? ' has-error' : '' }}">
                    ...
                    <div class="input-group">
                        {{ Form::number($attribute->slug.'[]', ! $entity->exists ? $attribute->default : $value, ['class' => 'form-control', 'placeholder' => $attribute->name, 'required' => $attribute->is_required ? 'required' : false]) }}
    ...
    <div class="row">
        ...
        <div class="col-md-12">
            ...
            <div class="form-group{{ $errors->has($attribute->slug) ? ' has-error' : '' }}">
                {{ Form::label($attribute->slug, $attribute->name, ['class' => 'control-label']) }}
                {{ Form::number($attribute->slug, ! $entity->exists ? $attribute->default : $entity->{$attribute->slug}, ['class' => 'form-control', 'placeholder' => $attribute->name, 'required' => $attribute->is_required ? 'required' : false]) }}
