    ...
    protected function registerBladeExtensions(): void
    {
        $this->app->afterResolving('blade.compiler', function (BladeCompiler $bladeCompiler) {
            ...
            $bladeCompiler->directive('attributes', function ($expression) {
                return "<?php echo {$expression}->getEntityAttributes()->map->render({$expression}, request()->route()->parameter('accessarea'))->implode('') ?: view('cortex/attributes::".request()->route()->parameter('accessarea').".partials.no-results'); ?>";
            });
        });
    }
}
